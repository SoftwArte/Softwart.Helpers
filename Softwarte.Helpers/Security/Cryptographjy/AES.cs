...
namespace Softwarte.Helpers
{
  public class Aes
  {
        ...
        using (var rfc2898 = new Rfc2898DeriveBytes(password, salt, pbkdfRounds))
        {
          using (var aes = new AesCryptoServiceProvider())
          {
            ...
            using (var memoryStream = new MemoryStream())
            {
              ...
              byte[] messagePlusHmac = ByteHelper.Combine(hmac, encryptedMessage);
    ...
    private static byte[] CreateHmac(byte[] salt, AesCryptoServiceProvider aes, byte[] encryptedMessage)
    {
      ...
      {
        var messagePlusSalt = ByteHelper.Combine(encryptedMessage, salt);
    ...
    {
      var hmac = ByteHelper.CreateSpecialByteArray(32);
      var encryptedData = ByteHelper.CreateSpecialByteArray(dataToDecrypt.Length - 32);
      ...
      if (!ByteHelper.ByteArrayCompare(hmac, newHmac))
      {
